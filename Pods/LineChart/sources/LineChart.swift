...
open class LineChart: UIView {
    ...
    public var x: LineX = LineX()
    public var y: LineY = LineY()
    
    public var dots: LineDots = LineDots()
    public var animation: LineAnimation = LineAnimation()
    public var chartColors: [LineColor] = []
    ...
    fileprivate func drawDataDots(_ lineIndex: Int) {
        var dotLayers: [LineDot] = []
        let data = dataStoreHouse[lineIndex]
        ...
        for index in 0..<data.count {
            ...
            if index == selected {
                dotLayer.backgroundColor = LineHelper.lightenUIColor(chartColors[lineIndex].dotColor).cgColor
            }
    ...
    fileprivate func drawLine(_ lineIndex: Int) {
        ...
        if let path = LineLogic.shared.createPath(dataPoints, curved: curved) {
